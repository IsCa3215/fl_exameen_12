...
Future<Map<String, dynamic>?> getUserData(String uid) async {
  final doc =
      await FirebaseFirestore.instance.collection('usuarios_ismael').doc(uid).get();
  return doc.data();
}

Future<void> updateUserWeigth(String uid, double newWeight) async {
  await db.collection('usuarios_ismael').doc(uid).update({'peso': newWeight});
}

Future<void> updateUser(String uid, String email, String nombre,
    String apellido, double newHeight) async {
  await db.collection('usuarios_ismael').doc(uid).update({
    'email': email,
    'nombre': nombre,
    'apellido': apellido,
    'peso': newHeight
...
Future<List<Usuario>> getAllUsers() async {
  final QuerySnapshot snapshot = await db.collection('usuarios_ismael').get();
