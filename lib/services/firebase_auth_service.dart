...
// Traduce los codigos de error de firebase a español
  ...
  Future<String?> registerUser({
    ...
    try {
      ...
      await _firestore.collection('usuarios_ismael').doc(uid).set({
...
//Si el usuario no existe en la base de datos lo crea
      if (user != null) {
        DocumentSnapshot userDoc =
            await _firestore.collection('usuarios_ismael').doc(user.uid).get();
        if (!userDoc.exists) {
          await _firestore.collection('usuarios_ismael').doc(user.uid).set({
